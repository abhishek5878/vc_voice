...
- **Auth:** Passcode only. Sign-in page (`/auth`) has a single passcode field. Correct passcode (env `ROBIN_PASSCODE`, default `42`) sets a cookie; the client then signs in with Supabase **anonymous auth** so each visitor gets their own user (profile, pitch link, deals). No email, password, or Vercel user setup. Enable **Anonymous sign-ins** in Supabase Dashboard → Authentication → Providers.
